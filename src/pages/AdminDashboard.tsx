...
import { Users, ShieldCheck, Package, ShoppingCart, Search, Key, Trash2, UserCheck, UserX, Eye, LogOut } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const handleToggleSellerRole = async (userId: string) => {
    const hasSellerRole = userRoles[userId]?.includes("seller");
    ...
    if (hasSellerRole) {
      ...
      if (error) {
        ...
        toast({ title: "Thành công", description: "Đã thu quyền seller" });
        fetchAllData();
      }
    } else {
      // Grant seller role
      ...
      if (error) {
        ...
        toast({ title: "Thành công", description: "Đã cấp quyền seller" });
        fetchAllData();
      }
    }
  };
  ...
  const handleDeleteCartItem = async (cartItemId: string) => {
    ...
    if (error) {
      ...
      toast({ title: "Thành công", description: "Đã xóa sản phẩm khỏi giỏ hàng" });
    }
  };
                          ...
                          <div className="flex gap-1 flex-wrap">
                            ...
                            {!userRoles[user.id]?.includes("admin") && (
                              <Button
                                ...
                                onClick={() => handleToggleSellerRole(user.id)}
