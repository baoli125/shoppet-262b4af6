  ...
  return (
    <section className="py-12 md:py-20 bg-card">
      <div className="container mx-auto px-4">
        ...
        <div 
          ref={elementRef}
          className={`grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 transition-all duration-700 ${
        ...
        >
          {features.map((feature, index) => (
            <div
              key={index}
              className="card-modern text-center group hover:shadow-xl"
              style={{ transitionDelay: `${index * 0.1}s` }}
            >
              <div className={`inline-flex items-center justify-center w-12 h-12 md:w-16 md:h-16 rounded-full ${feature.bgColor} mb-3 md:mb-4 group-hover:scale-110 transition-transform duration-300`}>
                <feature.icon className={`w-6 h-6 md:w-8 md:h-8 ${feature.color}`} />
              </div>
              <h3 className="text-base md:text-xl font-semibold mb-2 md:mb-3 text-foreground group-hover:text-primary transition-colors">
                {feature.title}
              </h3>
